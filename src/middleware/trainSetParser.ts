...
function trainSetParser(reject = true, req: TMTrainSetRequest, res: Response, next: NextFunction) {
	const realm = req.realm;
	const setId = req.params['trainset'];
	if (!setId && reject) return res.status(404).send({ message: `Missing train set ID in request URL` });
	const set = realm.trainSetManager.get(setId);
	if (!set && reject) return res.status(404).send({ message: `Train set does not exist` });
