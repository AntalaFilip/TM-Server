...
function locoParser(reject = true, req: TMLocoRequest, res: Response, next: NextFunction) {
	const realm = req.realm;
	const locoId = req.params['locomotive'];
	if (!locoId && reject) return res.status(404).send({ message: `Missing locomotive ID in request URL` });
	const loco = realm.movableManager.getLoco(locoId);
	if (!loco && reject) return res.status(404).send({ message: `Locomotive does not exist` });
